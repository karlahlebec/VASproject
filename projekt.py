...
class SenderAgent(Agent):
    class SendMessageBehaviour(CyclicBehaviour):
        async def run(self):
            ...
            msg = Message(to="prviagent@localhost")  # Replace with the JID of the DmAgent
            msg.set_metadata("performative", "inform")
            msg.body = input("Unesite proizvod za koji zelite znati cijenu u Dm-u: ")  # Replace with the product name

            await self.send(msg)
            print(f"Sent a message to {msg.to} about product: {msg.body}")
            self.kill()  # Stop this behaviour after sending the message
...
class SenderAgent2(Agent):
    class SendMessageBehaviour(CyclicBehaviour):
        async def run(self):
            await asyncio.sleep(5)  # Delay before sending message
            msg = Message(to="drugiagent@localhost")  # Replace with the JID of the DmAgent
            msg.set_metadata("performative", "inform")
            msg.body = input("Unesite proizvod za koji zelite znati cijenu u Bipi: ")  # Replace with the product name
    ...
    class CreateShoppingListBehaviour(CyclicBehaviour):
        ...
        async def run(self):
            ...
            if msg:
                ...
                if store == "Dm-u" and product not in self.dm_products:  #osigurava da se samo jednom ispiše na listu a ne dvaput
                    self.dm_products.append(product)
                elif store == "Bipi" and product not in self.bipa_products:
                    self.bipa_products.append(product)

                # After receiving 6 products, print the shopping lists
                if len(self.dm_products) + len(self.bipa_products) >= 6:
                    ...
                    #self.kill()  # Stop after processing 6 products

    class HandleUnmatchedMessages(CyclicBehaviour):
        async def run(self):
            msg = await self.receive(timeout=10)
            if msg:
                print(f"Received unmatched message: {msg.body}") #na kraju obriši ovaj mesage body
